...
@interface BBGuidePageView : UIView<UIScrollViewDelegate>

typedef void(^TMGuidePageCompleteBlock)(void);
...
@property (nonatomic,copy) TMGuidePageCompleteBlock completeBlock;
 ...
 *  @brief 引导页view创建
     ...
     //创建引导页视图
     BBGuidePageView *pageView = [BBGuidePageView initWithFrame:self.view.bounds WithImages:images completeBlock:^{
     NSLog(@"进入app主页");
     }];
     //假如是tabbarcontroller,就添加到tabbarcontroller.view中,
