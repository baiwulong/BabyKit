...
#define BBScreen_width   [[UIScreen mainScreen] bounds].size.width
#define BBDefaultRect     CGRectMake(0, 0, BBScreen_width, BBScreen_height)

#define BBDefaultView [[UIApplication sharedApplication] keyWindow]
...
/* 默认网络提示，可在这统一修改 */
static NSString *const BBLoadingMessage = @"加载中";

/* 默认简短提示语显示的时间，在这统一修改 */
static CGFloat const   BBShowTime  = 2.0f;

/* 手势是否可用，默认yes，轻触屏幕提示框隐藏 */
static BOOL isAvalibleTouch = YES;

@interface BBShowManager()

@end


@implementation BBShowManager
UIView *gloomyView;//深色背景
UIView *prestrainView;//预加载view
BOOL isShowGloomy;//是否显示深色背景
#pragma mark -   类初始化
+ (void)initialize {
    if (self == [BBShowManager self]) {
        //该方法只会走一次
        [self customView];
    }
}
...
+(void)customView {
    gloomyView = [[GloomyView alloc] initWithFrame:BBDefaultRect];
...
+ (void) showBriefAlert:(NSString *) message inView:(UIView *) view {
    [self showBriefAlert:message time:BBShowTime inView:view isHerizotal:NO];
}
...
+ (void)showBriefAlert:(NSString *)message time:(NSInteger)showTime inView:(UIView *)view isHerizotal:(BOOL)isHerizontal {
    dispatch_async(dispatch_get_main_queue(), ^{
        prestrainView = view;
        MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:view ?:BBDefaultView animated:YES];
...
#pragma mark - 长时间的提示语
+ (void) showPermanenBBessage:(NSString *)message inView:(UIView *) view{
    dispatch_async(dispatch_get_main_queue(), ^{
        prestrainView = view;
...
+ (void) showLoadingInView:(UIView *) view{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        hud.label.text = BBLoadingMessage;
        hud.removeFromSuperViewOnHide = YES;
        gloomyView.frame = view ? CGRectMake(0, 0, view.frame.size.width, view.frame.size.height):
        BBDefaultRect;
        if (isShowGloomy) {
...
+(void)showAlertWithCustomImage:(NSString *)imageName title:(NSString *)title inView:(UIView *)view{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        hud.mode = MBProgressHUDModeCustomView;
        [hud showAnimated:YES];
        [hud hideAnimated:YES afterDelay:BBShowTime];
...
+(void)showPermanentAlert:(NSString *)alert{
    [self showPermanenBBessage:alert inView:nil];
}
...
+(void)hideAlert{
    dispatch_async(dispatch_get_main_queue(), ^{
        MBProgressHUD *hud = [BBShowManager HUDForView:gloomyView];
#if 1
