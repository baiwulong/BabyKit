...
{
    if (!_dispathQueue) {
        _dispathQueue = dispatch_queue_create("BBFileHelper", NULL);
    }
...
{
    NSFileManager *BBFileHelper = [NSFileManager defaultManager];
    NSError *error;
    BOOL succeed = [BBFileHelper removeItemAtPath:path error:&error];
    return succeed;
}
...
{
    NSFileManager *BBFileHelper = [NSFileManager defaultManager];
    return [BBFileHelper fileExistsAtPath:path];
}
...
+ (BOOL)fileExistsAtDocumentPathWithFileName:(NSString *)fileName{
    ...
    return [BBFileHelper fileExistsAtPath:filePath];
}
