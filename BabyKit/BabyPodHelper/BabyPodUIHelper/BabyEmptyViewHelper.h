...
/** 空白页点击回调 */
typedef void(^BabyEmptyOnClick)(void);

/**
 --------------------------------------------------------------
 `BabyEmptyConfig`空白页面的配置
 配置图片，标题，详情描述，可自行配置文字颜色属性等
 --------------------------------------------------------------
 */
@interface BabyEmptyConfig : NSObject
...
/// 空白页面-点击按钮事件配置
@property (nonatomic,copy) BabyEmptyOnClick buttonOnClick ;
/// 空白页面-点击view事件配置
@property (nonatomic,copy) BabyEmptyOnClick viewOnClick ;
...
/**
 *  @brief 获取默认配置的一个BabyEmptyConfig对象，然后可以修改对象的属性
 */
...
/**
 --------------------------------------------------------------
 `BabyEmptyViewHelper`基于`DZNEmptyDataSet`封装的空白页面管理类
 根据传递的类型创建不同类型的空白页面
 --------------------------------------------------------------
 */
@interface BabyEmptyViewHelper : NSObject

/// 空白页面配置
@property (nonatomic,strong) BabyEmptyConfig *config ;

/**
 *  需要手动创建一个config对象，可以直接通过 `+(BabyEmptyConfig *)defaultConfig`类方法来创建，并修改
 *
 *  @brief 创建一个空白页面,注意，返回对象BabyEmptyViewHelper需要在控制器中设为全局属性保留，不然显示不出来

     //实例代码：
     BabyEmptyConfig *config = [BabyEmptyConfig defaultConfig];
     self.manager = [BabyEmptyViewHelper emptyView:self.tableview config:config buttonOnClick:^{
 ...
 *
 *  @return BabyEmptyViewHelper 管理空白页的管理器，负责重新加载显示或消失管理
 *
 *  @since 0.1.0
 */
+(BabyEmptyViewHelper *)emptyView:(UIScrollView *)emptyView
                          config:(BabyEmptyConfig *)config
