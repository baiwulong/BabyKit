...
@interface BabyEmptyViewHelper()<DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
...
@implementation BabyEmptyConfig

#pragma mark - 设置默认config对象

+(BabyEmptyConfig *)defaultConfig{
    
    BabyEmptyConfig *config = [[BabyEmptyConfig alloc]init];
    ...
    //标题文本属性
    config.contentTitle = @"默认显示的标题，需要替换`BabyEmptyConfig`对象中的`contentTitle`";
    ...
    //详情文本属性
    config.contentDetail = @"默认显示的详细描述，需要替换`BabyEmptyConfig`对象中的`contentDetail`";
...
/**
 --------------------------------------------------------------
 `BabyEmptyViewHelper`空白页面-管理器
 --------------------------------------------------------------
 */

@implementation BabyEmptyViewHelper

+(BabyEmptyViewHelper *)emptyView:(UIScrollView *)emptyView config:(BabyEmptyConfig *)config buttonOnClick:(BabyEmptyOnClick)buttonOnClick viewOnClick:(BabyEmptyOnClick)viewOnClick
{
    BabyEmptyViewHelper *manager = [[BabyEmptyViewHelper alloc]init];
    //去除分割线技巧
    ...
    //设置config
    manager.config = config != nil ? config : [BabyEmptyConfig defaultConfig];
...
+(BabyEmptyViewHelper *)emptyView:(UIScrollView *)emptyView
                contentImageName:(id)contentImageName
                 buttonImageName:(id)buttonImageName
                           title:(NSString *)title
                          detail:(NSString *)detail
                   buttonOnClick:(BabyEmptyOnClick)buttonOnClick
                     viewOnClick:(BabyEmptyOnClick)viewOnClick
{
    ...
    BabyEmptyConfig *config = [BabyEmptyConfig defaultConfig];
    ...
    BabyEmptyViewHelper *manager = [self emptyView:emptyView config:config buttonOnClick:buttonOnClick viewOnClick:viewOnClick];
    return manager;
}
